...
describe('FilterTasksByPipe', () => {
  ...
  it('task has by status LOW and type TASK', () => {
    const pipe = new FilterTasksByPipe();
    filters = [
      {
        id: 0,
    ...
    expect(pipe.isTaskMatchesFilters(tasks[0], filters)).toEqual(false);
  });

  it('task has by status LOW and type ISSUE', () => {
    const pipe = new FilterTasksByPipe();
    filters = [
      {
        id: 0,
    ...
    expect(pipe.isTaskMatchesFilters(tasks[0], filters)).toEqual(true);
  });
});
